...
class PromptSection(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique identifier for this section instance (UUID).")
    typeId: str = Field(..., description="Identifier for the type of section (e.g., 'role', 'context'), maps to sectionTypes.id in frontend.")
    name: str = Field(..., description="Display name for the section type (e.g., '角色定义').")
    content: str = Field(..., description="Content of the section, can include Markdown and textual XML tag references.")
    order: int = Field(..., description="Order of this section in the prompt sequence.")
...
class PromptBase(BaseModel):
    ...
    sections: List[PromptSection] = Field(default_factory=list, description="The structured sections of the prompt, ordered by the 'order' field within each section.")
    text: Optional[str] = Field(None, description="Assembled XML text content of the prompt. Generated by the backend from sections.")
    project: Optional[str] = Field(None, description="Associated project identifier (e.g., 'hk4e', 'rpg', 'blhz', 'nxx', 'bh3').")
